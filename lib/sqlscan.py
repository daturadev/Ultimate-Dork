...
class sqli_scan(object):
      ...
      def error(self):
          return {
              ...
              "MICROSOFT-19":        r'\[Microsoft\]\[SQL Server Native Client 11\.0\]\[SQL Server\]',
              ...
              "PHP-ERROR-06":        r'Warning: include\(',
              "PHP-ERROR-07":        r'Warning: require_once\(',
              "PHP-ERROR-08":        'function.include',
              "PHP-ERROR-09":        'Disallowed Parent Path',
              "PHP-ERROR-10":        'function.require',
              "PHP-ERROR-11":        r'Warning: main\(',
              "PHP-ERROR-12":        r'Warning: session_start\(\)',
              "PHP-ERROR-13":        r'Warning: getimagesize\(\)',
              "PHP-ERROR-14":        r'Warning: array_merge\(\)',
              "PHP-ERROR-15":        r'Warning: preg_match\(\)',
              "PHP-ERROR-16":        r'GetArray\(\)',
              "PHP-ERROR-17":        r'FetchRow\(\)',
              "PHP-ERROR-18":        'Warning: preg_',
              "PHP-ERROR-19":        r'Warning: ociexecute\(\)',
              "PHP-ERROR-20":        r'Warning: ocifetchstatement\(\)',
              "ASP-ERROR-01":        r'Version Information: Microsoft \.NET Framework',
      ...
      def scan(self, url, proxy=None, output_cb=None):
          ...
          for a, b in self.error.items():
              if re.search(b, resp):
                  with open(f'{pth()}/vuln.txt', 'a+') as f:
                      f.write(f'{self.url}\n')
                  _vuln += 1
                  __vuln += b

          if _vuln > 0:
              output_cb(f'\n{_.G}[!]{_.W} Vulnerability SQLi')
              output_cb(f'{_.Y}[E]{_.W} : {__vuln}')
              output_cb(f'{_.B}[URL]{_.W} : {self.url}')
              output_cb(f'Save To : {pth()}/vuln.txt\n')
          else:
              output_cb(f'{_.R}[-]{_.W} Not Vulnerability')
              output_cb(f'{_.B}[URL]{_.W} : {self.url}')
